...
function CreatorDashboard() {
    ...
    // Process PDF via API (SOLID: Calls backend pipeline, not browser JS)
    const API_BASE = import.meta.env.DEV ? 'http://localhost:8000' : ''

    // Process PDF Client-Side (Vercel Compatible)
    const handleGenerate = async () => {
        setStep(3)
        setProcessing({ status: 'parsing', progress: 10, currentStep: 'Analyzing PDF structure...', results: null })

        try {
            // Give UI a moment to update
            await new Promise(r => setTimeout(r, 500))

            console.log('[Creator] Starting Client-Side Processing...')
            const result = await processPdfToCourse(pdfFile)

            if (!result.success) {
                throw new Error(result.error)
            }

            setProcessing(p => ({ ...p, progress: 50, currentStep: 'Generating Lesson Plan...' }))
            await new Promise(r => setTimeout(r, 800)) // Fake delay for UX

            // Format for display
            const finalResults = {
                courseId: `course-${Date.now()}`,
                title: config.title || result.lessons[0]?.title || 'New Course',
                unitsCount: result.unitsDetected || 1,
                ...
                units: result.lessons.reduce((acc, l) => {
                    const u = acc.find(u => u.number === l.unit?.number)
                    if (u) { u.lessons++ }
                    else { acc.push({ number: l.unit?.number || 1, title: l.unit?.title || 'General', lessons: 1 }) }
                    return acc
                }, []),
                rawData: result
            }
